...
from market_scanner.models import Candle, EquityQuote, MarketRegime, ScanResult, ScannerCandidate, TickerMetrics
...
def _test_proposal(proposal_id: str, symbol: str) -> OptionProposal:
    return OptionProposal(
        ...
        direction="long",
        structure="single",
        created_at=datetime(2026, 6, 8, 13, 30, tzinfo=timezone.utc),
        ...
        legs=[
            OptionProposalLeg(
                ...
                qty=1,
                symbol=symbol,
                broker_symbol=f"{symbol}  260612C00100000",
                expiry=date(2026, 6, 12),
                strike=100,
                right="CALL",
                price=2.0,
        ...
        debit=200,
        max_loss=200,
        natural_limit_price=2.0,
        natural_debit=200,
        send_limit_price=2.1,
    )
...
def test_selected_proposal_build_preserves_other_built_symbols():
    ...
    scanner._candidate_with_proposals = MethodType(fake_candidate_with_proposals, scanner)
    candidates = [
        ScannerCandidate(
            rank=1,
            symbol="MU",
            ...
            direction="long",
            score=90,
            metrics=TickerMetrics(symbol="MU", current_price=100),
            proposals=[_test_proposal("old_mu", "MU")],
        ),
        ScannerCandidate(
            rank=2,
            symbol="AMD",
            ...
            direction="long",
            score=80,
            metrics=TickerMetrics(symbol="AMD", current_price=100),
        ),
    ]
    result = ScanResult(
        scan_id="scan_test",
        scanned_at=datetime(2026, 6, 8, 13, 30, tzinfo=timezone.utc),
        session="regular",
        universe=["MU", "AMD"],
        regime=MarketRegime(bias="bullish", score=1, symbols=[]),
        candidates=candidates,
        top_candidates=candidates,
        notes=["Generated by Schwab Market Scanner."],
    )

    updated = scanner.build_selected_candidate_proposals(result, "AMD")
